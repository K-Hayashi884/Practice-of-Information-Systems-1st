...
def link1_to_link2(link1_list):
    ...
    for link1 in link1_list:
        if "%E3%83%A9%E3%82%A4%E3%83%95" in link1[0]: #ライフの場合
            r = requests.get(link1[0],headers=headers)
            ...
            for element in elements:
                ...
                if period < 6: #ライフでは一般的に掲載期間が6日以上のチラシは取得したくないチラシなので
                    link2_list.append((l+link,link1[1]))
        elif "%E3%82%B0%E3%83%AC%E3%83%BC%E3%82%B9%E3%81%9F%E3%81%AA%E3%81%8B" in link1[0]: #グレースたなかの場合
            r = requests.get(link1[0],headers=headers)
            soup = BeautifulSoup(r.text,"html.parser")
            elements = soup.find_all(class_="image_element")
            for element in elements:
...
def update_url_list(link2_list):
    """
    `link2_list`に含まれる最新でないURLを、./urls.txtを利用し削除
    ...
    for link2 in link2_list:
        if link2[0] in old_link2_list: #昨日使用したリンクは無視
            continue
        else: #昨日なかったリンクは欲しいのでnew_link2_listに格納
            new_link2_list.append(link2)
            
    urls_new = map(lambda x: x[0] + "\n", link2_list)
    #link2_listの内容をurls.txtに保存
...
def get_url(new_link2_list):
  ...
  for new_link2 in new_link2_list:
    r = requests.get(new_link2[0],headers=headers)
    soup = BeautifulSoup(r.text,"html.parser")
    link3_url = soup.find('img',class_='leaflet').get('src').split("?")[0]
    link3_list.append((link3_url,new_link2[1])) 
  return link3_list
...
def get_bargains(link1_list):
    ...
    for link1 in link1_list:
        ...
        if "%E3%82%B3%E3%83%AC%E3%83%A2/187995" in l: #コレモ出町柳の場合
            ...
            sales_list.append(([element.contents[0][1:-1] for element in elements],link1[1]))
...
#　これを受け取った後、type1の店リストとtype2の店リストの2つに振り分け、それぞれ

input_list = []
input_list.append(("グレースたなか","https://tokubai.co.jp/%E3%82%B0%E3%83%AC%E3%83%BC%E3%82%B9%E3%81%9F%E3%81%AA%E3%81%8B/12265",0))
input_list.append(("ライフ北白川","https://tokubai.co.jp/%E3%83%A9%E3%82%A4%E3%83%95/9470",0))
input_list.append(("大国屋白川店","https://tokubai.co.jp/%E5%A4%A7%E5%9B%BD%E5%B1%8B/14801",1))
input_list.append(("コレモ出町柳","https://tokubai.co.jp/%E3%82%B3%E3%83%AC%E3%83%A2/187995",1))
input_list.append(("フレスコ河原町丸太町","https://tokubai.co.jp/%E3%83%95%E3%83%AC%E3%82%B9%E3%82%B3/4244",1))
input_list.append(("イズミヤ高野店","https://tokubai.co.jp/%E3%82%A4%E3%82%BA%E3%83%9F%E3%83%A4/9148",1))

type1_link1_list = []
type2_link1_list = []
for t in input_list:
    if t[2] == 0:
        type1_link1_list.append((t[1],t[0]))
    else:
        type2_link1_list.append((t[1],t[0]))
